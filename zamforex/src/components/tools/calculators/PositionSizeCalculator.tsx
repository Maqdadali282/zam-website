...
export default function PositionSizeCalculator() {
  ...
  const [stopLossPrice, setStopLossPrice] = useState<number | "">(ILLUSTRATIVE_PRICES["EUR/USD"] - 0.003);
  ...
  function handleCalculate() {
    ...
    if (needsManualRate && (manualRate === "" || manualRate <= 0)) {
      return fail(`Enter a ${pair.quote}/${accountCurrency} conversion rate below.`);
    }
    ...
    <div className="calc-grid">
      <div className="calc-panel">
        ...
        <CalculatorInput
          ...
          unit="%"
          hint="How much of your account you're willing to risk on this one trade."
          min={0}
        />
        ...
        {needsManualRate && (
          <CalculatorInput
            ...
            hint={`We don't have a live rate feed — enter the current ${pair.quote}/${accountCurrency} rate from your platform for an accurate figure.`}
          />
        )}
        {error && (
      ...
      <div className="calc-panel">
        <h2>Results</h2>
        <ResultHeadline label="Recommended Position Size" value={result ? `${formatLots(result.recommendedLots)} lots` : undefined} accent={ACCENT} placeholder="Enter your details and calculate" />
        <ResultMetricGrid>
          ...
          <ResultMetric label="Account Balance" value={accountBalance !== "" ? formatMoney(accountBalance, accountCurrency) : "—"} />
        ...
        <div className="result-visual">
          ...
          <MiniFlow
            accent={ACCENT}
            steps={[
              { label: "Account Balance", value: accountBalance !== "" ? formatMoney(accountBalance, accountCurrency) : "—" },
              { label: "Risk Percentage", value: riskPercent !== "" ? `${riskPercent}%` : "—" },
              { label: "Stop Loss Distance", value: result ? formatPips(result.stopLossPips) : "—" },
              { label: "Recommended Position Size", value: result ? `${formatLots(result.recommendedLots)} lots` : "—" },
            ]}
          />
        </div>
      </div>
